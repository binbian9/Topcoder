...
int	J;
		...
		__in >> __expected >> students >> Y >> J;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << students << ", " << Y << ", " << J << std::endl;
		std::cout.flush();

		__timer_start();

		TheJediTestDiv2 __object;
		int __actual = __object.countSupervisors(students, Y, J);
