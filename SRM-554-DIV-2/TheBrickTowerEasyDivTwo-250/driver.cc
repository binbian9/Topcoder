...
#include "TheBrickTowerEasyDivTwo.cc"
...
int	blueHeight;
		...
		__in >> __expected >> redCount >> redHeight >> blueCount >> blueHeight;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << redCount << ", " << redHeight << ", " << blueCount << ", " << blueHeight << std::endl;
		std::cout.flush();

		__timer_start();

		TheBrickTowerEasyDivTwo __object;
		int __actual = __object.find(redCount, redHeight, blueCount, blueHeight);
