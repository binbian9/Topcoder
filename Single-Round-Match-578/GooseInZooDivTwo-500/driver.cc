...
int	dist;
		...
		__in >> __expected >> field >> dist;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << field << ", " << dist << std::endl;
		std::cout.flush();

		__timer_start();

		GooseInZooDivTwo __object;
		int __actual = __object.count(field, dist);
