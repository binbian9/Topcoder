...
vector <string>	R;
		...
		__in >> __expected >> N >> L >> R;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << L << ", " << R << std::endl;
		std::cout.flush();

		__timer_start();

		WolfInZooDivTwo __object;
		int __actual = __object.count(N, L, R);
