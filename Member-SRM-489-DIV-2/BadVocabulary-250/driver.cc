...
vector <string>	vocabulary;
		...
		__in >> __expected >> badPrefix >> badSuffix >> badSubstring >> vocabulary;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << badPrefix << ", " << badSuffix << ", " << badSubstring << ", " << vocabulary << std::endl;
		std::cout.flush();

		__timer_start();

		BadVocabulary __object;
		int __actual = __object.count(badPrefix, badSuffix, badSubstring, vocabulary);
