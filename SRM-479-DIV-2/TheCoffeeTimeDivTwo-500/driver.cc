...
#include "TheCoffeeTimeDivTwo.cc"
...
vector <int>	tea;
		...
		__in >> __expected >> n >> tea;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << tea << std::endl;
		std::cout.flush();

		__timer_start();

		TheCoffeeTimeDivTwo __object;
		int __actual = __object.find(n, tea);
