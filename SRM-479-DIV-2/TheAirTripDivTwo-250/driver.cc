...
int	fuel;
		...
		__in >> __expected >> flights >> fuel;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << flights << ", " << fuel << std::endl;
		std::cout.flush();

		__timer_start();

		TheAirTripDivTwo __object;
		int __actual = __object.find(flights, fuel);
