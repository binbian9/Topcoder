...
#include "SurroundingGameEasy.cc"
...
vector <string>	stone;
		...
		__in >> __expected >> cost >> benefit >> stone;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cost << ", " << benefit << ", " << stone << std::endl;
		std::cout.flush();

		__timer_start();

		SurroundingGameEasy __object;
		int __actual = __object.score(cost, benefit, stone);
