...
vector <int>	hp;
		...
		__in >> __expected >> dps >> hp;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dps << ", " << hp << std::endl;
		std::cout.flush();

		__timer_start();

		GreatFairyWar __object;
		int __actual = __object.minHP(dps, hp);
