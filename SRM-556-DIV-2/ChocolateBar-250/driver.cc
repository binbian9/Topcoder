...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> letters;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << letters << std::endl;
		std::cout.flush();

		__timer_start();

		ChocolateBar __object;
		int __actual = __object.maxLength(letters);
