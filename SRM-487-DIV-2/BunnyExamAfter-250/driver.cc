...
string	white;
		...
		__in >> __expected >> black >> gray >> white;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << black << ", " << gray << ", " << white << std::endl;
		std::cout.flush();

		__timer_start();

		BunnyExamAfter __object;
		int __actual = __object.getMaximum(black, gray, white);
