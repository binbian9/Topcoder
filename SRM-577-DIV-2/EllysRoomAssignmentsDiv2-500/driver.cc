...
#include "EllysRoomAssignmentsDiv2.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> ratings;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << ratings << std::endl;
		std::cout.flush();

		__timer_start();

		EllysRoomAssignmentsDiv2 __object;
		double __actual = __object.getProbability(ratings);
