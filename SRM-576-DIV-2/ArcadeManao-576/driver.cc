...
int	coinColumn;
		...
		__in >> __expected >> level >> coinRow >> coinColumn;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << level << ", " << coinRow << ", " << coinColumn << std::endl;
		std::cout.flush();

		__timer_start();

		ArcadeManao __object;
		int __actual = __object.shortestLadder(level, coinRow, coinColumn);
