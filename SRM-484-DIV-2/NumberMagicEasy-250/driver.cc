...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> answer;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << answer << std::endl;
		std::cout.flush();

		__timer_start();

		NumberMagicEasy __object;
		int __actual = __object.theNumber(answer);
