...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> numList;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << numList << std::endl;
		std::cout.flush();

		__timer_start();

		AverageAverage __object;
		double __actual = __object.average(numList);
