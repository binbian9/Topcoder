...
string	givenName;
		...
		__in >> __expected >> familyName >> givenName;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << familyName << ", " << givenName << std::endl;
		std::cout.flush();

		__timer_start();

		TopFox __object;
		int __actual = __object.possibleHandles(familyName, givenName);
