...
#include "ThreeColorabilityEasy.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> cells;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cells << std::endl;
		std::cout.flush();

		__timer_start();

		ThreeColorabilityEasy __object;
		string __actual = __object.isColorable(cells);
