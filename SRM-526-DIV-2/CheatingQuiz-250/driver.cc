...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> answers;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << answers << std::endl;
		std::cout.flush();

		__timer_start();

		CheatingQuiz __object;
		vector <int> __actual = __object.howMany(answers);
