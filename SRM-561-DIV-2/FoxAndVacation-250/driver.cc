...
vector <int>	d;
		...
		__in >> __expected >> total >> d;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << total << ", " << d << std::endl;
		std::cout.flush();

		__timer_start();

		FoxAndVacation __object;
		int __actual = __object.maxCities(total, d);
