...
vector <string>	votes;
		...
		__in >> __expected >> names >> votes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << names << ", " << votes << std::endl;
		std::cout.flush();

		__timer_start();

		RabbitVoting __object;
		string __actual = __object.getWinner(names, votes);
