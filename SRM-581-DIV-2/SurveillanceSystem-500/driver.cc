...
int	L;
		...
		__in >> __expected >> containers >> reports >> L;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << containers << ", " << reports << ", " << L << std::endl;
		std::cout.flush();

		__timer_start();

		SurveillanceSystem __object;
		string __actual = __object.getContainerInfo(containers, reports, L);
