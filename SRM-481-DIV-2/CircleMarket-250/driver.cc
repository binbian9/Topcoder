...
int	travelTime;
		...
		__in >> __expected >> openTime >> closeTime >> travelTime;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << openTime << ", " << closeTime << ", " << travelTime << std::endl;
		std::cout.flush();

		__timer_start();

		CircleMarket __object;
		int __actual = __object.makePurchases(openTime, closeTime, travelTime);
