...
int	liarCount;
		...
		__in >> __expected >> n >> eggCount >> lieCount >> liarCount;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << eggCount << ", " << lieCount << ", " << liarCount << std::endl;
		std::cout.flush();

		__timer_start();

		ChickenOracle __object;
		string __actual = __object.theTruth(n, eggCount, lieCount, liarCount);
