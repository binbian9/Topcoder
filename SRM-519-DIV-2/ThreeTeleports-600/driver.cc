...
vector <string>	teleports;
		...
		__in >> __expected >> xMe >> yMe >> xHome >> yHome >> teleports;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << xMe << ", " << yMe << ", " << xHome << ", " << yHome << ", " << teleports << std::endl;
		std::cout.flush();

		__timer_start();

		ThreeTeleports __object;
		int __actual = __object.shortestDistance(xMe, yMe, xHome, yHome, teleports);
