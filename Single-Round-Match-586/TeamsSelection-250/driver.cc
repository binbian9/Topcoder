...
vector <int>	preference2;
		...
		__in >> __expected >> preference1 >> preference2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << preference1 << ", " << preference2 << std::endl;
		std::cout.flush();

		__timer_start();

		TeamsSelection __object;
		string __actual = __object.simulate(preference1, preference2);
