...
#include "PiecewiseLinearFunctionDiv2.cc"
...
vector <int>	query;
		...
		__in >> __expected >> Y >> query;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << Y << ", " << query << std::endl;
		std::cout.flush();

		__timer_start();

		PiecewiseLinearFunctionDiv2 __object;
		vector <int> __actual = __object.countSolutions(Y, query);
