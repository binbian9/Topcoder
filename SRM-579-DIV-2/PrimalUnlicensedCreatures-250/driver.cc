...
#include "PrimalUnlicensedCreatures.cc"
...
vector <int>	grezPower;
		...
		__in >> __expected >> initialLevel >> grezPower;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << initialLevel << ", " << grezPower << std::endl;
		std::cout.flush();

		__timer_start();

		PrimalUnlicensedCreatures __object;
		int __actual = __object.maxWins(initialLevel, grezPower);
