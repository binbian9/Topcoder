...
int	value;
		...
		__in >> __expected >> matrix >> value;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << matrix << ", " << value << std::endl;
		std::cout.flush();

		__timer_start();

		MatrixShiftings __object;
		int __actual = __object.minimumShifts(matrix, value);
