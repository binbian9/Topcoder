...
#include "EasyConversionMachine.cc"
...
int	k;
		...
		__in >> __expected >> originalWord >> finalWord >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << originalWord << ", " << finalWord << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		EasyConversionMachine __object;
		string __actual = __object.isItPossible(originalWord, finalWord, k);
