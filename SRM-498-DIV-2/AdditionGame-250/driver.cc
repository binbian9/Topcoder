...
int	N;
		...
		__in >> __expected >> A >> B >> C >> N;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << B << ", " << C << ", " << N << std::endl;
		std::cout.flush();

		__timer_start();

		AdditionGame __object;
		int __actual = __object.getMaximumPoints(A, B, C, N);
