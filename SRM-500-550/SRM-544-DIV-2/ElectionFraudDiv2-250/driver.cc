...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> percentages;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << percentages << std::endl;
		std::cout.flush();

		__timer_start();

		ElectionFraudDiv2 __object;
		string __actual = __object.IsFraudulent(percentages);
