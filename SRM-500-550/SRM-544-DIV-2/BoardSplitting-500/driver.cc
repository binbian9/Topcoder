...
int	actualLength;
		...
		__in >> __expected >> desiredLength >> desiredCount >> actualLength;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << desiredLength << ", " << desiredCount << ", " << actualLength << std::endl;
		std::cout.flush();

		__timer_start();

		BoardSplitting __object;
		int __actual = __object.minimumCuts(desiredLength, desiredCount, actualLength);
