...
vector <string>	siblingGroups;
		...
		__in >> __expected >> femaleCompatibility >> maleCompatibility >> siblingGroups;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << femaleCompatibility << ", " << maleCompatibility << ", " << siblingGroups << std::endl;
		std::cout.flush();

		__timer_start();

		PlatypusPaternity __object;
		int __actual = __object.maxFamily(femaleCompatibility, maleCompatibility, siblingGroups);
