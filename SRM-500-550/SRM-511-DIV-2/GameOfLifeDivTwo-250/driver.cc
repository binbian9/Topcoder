...
int	T;
		...
		__in >> __expected >> init >> T;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << init << ", " << T << std::endl;
		std::cout.flush();

		__timer_start();

		GameOfLifeDivTwo __object;
		string __actual = __object.theSimulation(init, T);
