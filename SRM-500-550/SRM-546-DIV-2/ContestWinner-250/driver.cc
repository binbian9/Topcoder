...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> events;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << events << std::endl;
		std::cout.flush();

		__timer_start();

		ContestWinner __object;
		int __actual = __object.getWinner(events);
