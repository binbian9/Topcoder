...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> length;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << length << std::endl;
		std::cout.flush();

		__timer_start();

		MinimalTriangle __object;
		double __actual = __object.maximalArea(length);
