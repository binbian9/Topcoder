...
#include "MysteriousRestaurant.cc"
...
int	budget;
		...
		__in >> __expected >> prices >> budget;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << prices << ", " << budget << std::endl;
		std::cout.flush();

		__timer_start();

		MysteriousRestaurant __object;
		int __actual = __object.maxDays(prices, budget);
