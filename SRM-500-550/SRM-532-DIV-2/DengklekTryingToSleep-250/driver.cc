...
#include "DengklekTryingToSleep.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> ducks;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << ducks << std::endl;
		std::cout.flush();

		__timer_start();

		DengklekTryingToSleep __object;
		int __actual = __object.minDucks(ducks);
