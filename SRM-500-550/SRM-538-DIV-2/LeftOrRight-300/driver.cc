...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> program;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << program << std::endl;
		std::cout.flush();

		__timer_start();

		LeftOrRight __object;
		int __actual = __object.maxDistance(program);
