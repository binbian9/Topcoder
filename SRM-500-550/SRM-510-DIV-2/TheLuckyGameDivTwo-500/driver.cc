...
int	bLen;
		...
		__in >> __expected >> a >> b >> jLen >> bLen;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << a << ", " << b << ", " << jLen << ", " << bLen << std::endl;
		std::cout.flush();

		__timer_start();

		TheLuckyGameDivTwo __object;
		int __actual = __object.find(a, b, jLen, bLen);
