...
int	numSwaps;
		...
		__in >> __expected >> hats >> numSwaps;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << hats << ", " << numSwaps << std::endl;
		std::cout.flush();

		__timer_start();

		BallAndHats __object;
		int __actual = __object.getHat(hats, numSwaps);
