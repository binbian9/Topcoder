...
#include "YetAnotherIncredibleMachine.cc"
...
vector <int>	balls;
		...
		__in >> __expected >> platformMount >> platformLength >> balls;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << platformMount << ", " << platformLength << ", " << balls << std::endl;
		std::cout.flush();

		__timer_start();

		YetAnotherIncredibleMachine __object;
		int __actual = __object.countWays(platformMount, platformLength, balls);
