...
int	d2;
		...
		__in >> __expected >> maxR >> maxG >> maxB >> startR >> startG >> startB >> d1 >> d2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << maxR << ", " << maxG << ", " << maxB << ", " << startR << ", " << startG << ", " << startB << ", " << d1 << ", " << d2 << std::endl;
		std::cout.flush();

		__timer_start();

		RandomColoringDiv2 __object;
		int __actual = __object.getCount(maxR, maxG, maxB, startR, startG, startB, d1, d2);
