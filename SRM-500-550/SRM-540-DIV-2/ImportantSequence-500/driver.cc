...
string	operators;
		...
		__in >> __expected >> B >> operators;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << B << ", " << operators << std::endl;
		std::cout.flush();

		__timer_start();

		ImportantSequence __object;
		int __actual = __object.getCount(B, operators);
