...
int	BplusC;
		...
		__in >> __expected >> AminusB >> BminusC >> AplusB >> BplusC;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << AminusB << ", " << BminusC << ", " << AplusB << ", " << BplusC << std::endl;
		std::cout.flush();

		__timer_start();

		FoxAndIntegers __object;
		vector <int> __actual = __object.get(AminusB, BminusC, AplusB, BplusC);
