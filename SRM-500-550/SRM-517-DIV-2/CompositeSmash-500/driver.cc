...
int	target;
		...
		__in >> __expected >> N >> target;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << target << std::endl;
		std::cout.flush();

		__timer_start();

		CompositeSmash __object;
		string __actual = __object.thePossible(N, target);
