...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> moments;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << moments << std::endl;
		std::cout.flush();

		__timer_start();

		LuckyCounter __object;
		int __actual = __object.countLuckyMoments(moments);
