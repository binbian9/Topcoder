...
#include "PlatypusDuckAndBeaver.cc"
...
int	beaverTails;
		...
		__in >> __expected >> webbedFeet >> duckBills >> beaverTails;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << webbedFeet << ", " << duckBills << ", " << beaverTails << std::endl;
		std::cout.flush();

		__timer_start();

		PlatypusDuckAndBeaver __object;
		int __actual = __object.minimumAnimals(webbedFeet, duckBills, beaverTails);
