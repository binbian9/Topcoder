...
int	jackpot;
		...
		__in >> __expected >> money >> jackpot;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << money << ", " << jackpot << std::endl;
		std::cout.flush();

		__timer_start();

		TheJackpotDivTwo __object;
		vector <int> __actual = __object.find(money, jackpot);
