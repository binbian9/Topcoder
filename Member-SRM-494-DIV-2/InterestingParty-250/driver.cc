...
vector <string>	second;
		...
		__in >> __expected >> first >> second;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << first << ", " << second << std::endl;
		std::cout.flush();

		__timer_start();

		InterestingParty __object;
		int __actual = __object.bestInvitation(first, second);
