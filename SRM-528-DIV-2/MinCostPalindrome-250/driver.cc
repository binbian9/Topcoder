...
int	xCost;
		...
		__in >> __expected >> s >> oCost >> xCost;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << s << ", " << oCost << ", " << xCost << std::endl;
		std::cout.flush();

		__timer_start();

		MinCostPalindrome __object;
		int __actual = __object.getMinimum(s, oCost, xCost);
