...
#include "LittleElephantAndBooks.cc"
...
int	number;
		...
		__in >> __expected >> pages >> number;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << pages << ", " << number << std::endl;
		std::cout.flush();

		__timer_start();

		LittleElephantAndBooks __object;
		int __actual = __object.getNumber(pages, number);
