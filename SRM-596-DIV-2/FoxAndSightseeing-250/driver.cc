...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> position;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << position << std::endl;
		std::cout.flush();

		__timer_start();

		FoxAndSightseeing __object;
		int __actual = __object.getMin(position);
