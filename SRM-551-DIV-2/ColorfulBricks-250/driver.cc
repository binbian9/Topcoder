...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> bricks;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << bricks << std::endl;
		std::cout.flush();

		__timer_start();

		ColorfulBricks __object;
		int __actual = __object.countLayouts(bricks);
