...
#include "DengklekMakingChains.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> chains;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << chains << std::endl;
		std::cout.flush();

		__timer_start();

		DengklekMakingChains __object;
		int __actual = __object.maxBeauty(chains);
