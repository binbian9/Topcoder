...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> word;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << word << std::endl;
		std::cout.flush();

		__timer_start();

		FauxPalindromes __object;
		string __actual = __object.classifyIt(word);
