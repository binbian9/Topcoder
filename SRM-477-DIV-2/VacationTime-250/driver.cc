...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		int	N;
int	K;
vector <int>	workingDays;

		__in >> __expected >> N >> K >> workingDays;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << K << ", " << workingDays << std::endl;
		std::cout.flush();

		__timer_start();

		VacationTime __object;
		int __actual = __object.bestSchedule(N, K, workingDays);
