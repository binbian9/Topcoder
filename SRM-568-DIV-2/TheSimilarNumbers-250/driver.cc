...
int	upper;
		...
		__in >> __expected >> lower >> upper;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << lower << ", " << upper << std::endl;
		std::cout.flush();

		__timer_start();

		TheSimilarNumbers __object;
		int __actual = __object.find(lower, upper);
