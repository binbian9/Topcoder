...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> profit;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << profit << std::endl;
		std::cout.flush();

		__timer_start();

		WorkingRabbits __object;
		double __actual = __object.getEfficiency(profit);
