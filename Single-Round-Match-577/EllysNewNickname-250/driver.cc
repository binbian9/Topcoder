...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> nickname;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << nickname << std::endl;
		std::cout.flush();

		__timer_start();

		EllysNewNickname __object;
		int __actual = __object.getLength(nickname);
