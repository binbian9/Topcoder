...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> duckTypes;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << duckTypes << std::endl;
		std::cout.flush();

		__timer_start();

		KingdomAndDucks __object;
		int __actual = __object.minDucks(duckTypes);
