...
int	minuteHand;
		...
		__in >> __expected >> hourHand >> minuteHand;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << hourHand << ", " << minuteHand << std::endl;
		std::cout.flush();

		__timer_start();

		RotatedClock __object;
		string __actual = __object.getEarliest(hourHand, minuteHand);
