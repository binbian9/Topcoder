...
vector <int>	toId;
		...
		__in >> __expected >> capacities >> bottles >> fromId >> toId;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << capacities << ", " << bottles << ", " << fromId << ", " << toId << std::endl;
		std::cout.flush();

		__timer_start();

		KiwiJuiceEasy __object;
		vector <int> __actual = __object.thePouring(capacities, bottles, fromId, toId);
