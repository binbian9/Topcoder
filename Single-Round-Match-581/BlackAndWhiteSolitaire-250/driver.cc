...
#include "BlackAndWhiteSolitaire.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> cardFront;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cardFront << std::endl;
		std::cout.flush();

		__timer_start();

		BlackAndWhiteSolitaire __object;
		int __actual = __object.minimumTurns(cardFront);
