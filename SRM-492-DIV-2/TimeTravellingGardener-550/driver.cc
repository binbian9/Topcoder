...
#include "TimeTravellingGardener.cc"
...
vector <int>	height;
		...
		__in >> __expected >> distance >> height;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << distance << ", " << height << std::endl;
		std::cout.flush();

		__timer_start();

		TimeTravellingGardener __object;
		int __actual = __object.determineUsage(distance, height);
