...
#include "TimeTravellingCellar.cc"
...
vector <int>	decay;
		...
		__in >> __expected >> profit >> decay;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << profit << ", " << decay << std::endl;
		std::cout.flush();

		__timer_start();

		TimeTravellingCellar __object;
		int __actual = __object.determineProfit(profit, decay);
