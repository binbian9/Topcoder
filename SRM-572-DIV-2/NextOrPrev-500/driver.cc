...
string	goal;
		...
		__in >> __expected >> nextCost >> prevCost >> start >> goal;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << nextCost << ", " << prevCost << ", " << start << ", " << goal << std::endl;
		std::cout.flush();

		__timer_start();

		NextOrPrev __object;
		int __actual = __object.getMinimum(nextCost, prevCost, start, goal);
