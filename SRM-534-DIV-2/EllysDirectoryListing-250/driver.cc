...
#include "EllysDirectoryListing.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> files;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << files << std::endl;
		std::cout.flush();

		__timer_start();

		EllysDirectoryListing __object;
		vector <string> __actual = __object.getFiles(files);
