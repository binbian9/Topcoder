...
int	K;
		...
		__in >> __expected >> carrots >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << carrots << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		CarrotBoxesEasy __object;
		int __actual = __object.theIndex(carrots, K);
