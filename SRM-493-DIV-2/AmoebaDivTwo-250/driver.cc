...
int	K;
		...
		__in >> __expected >> table >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << table << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		AmoebaDivTwo __object;
		int __actual = __object.count(table, K);
