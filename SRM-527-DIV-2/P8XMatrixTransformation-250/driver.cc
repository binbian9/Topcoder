...
#include "P8XMatrixTransformation.cc"
...
vector <string>	target;
		...
		__in >> __expected >> original >> target;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << original << ", " << target << std::endl;
		std::cout.flush();

		__timer_start();

		P8XMatrixTransformation __object;
		string __actual = __object.solve(original, target);
