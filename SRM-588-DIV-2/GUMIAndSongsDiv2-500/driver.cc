...
int	T;
		...
		__in >> __expected >> duration >> tone >> T;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << duration << ", " << tone << ", " << T << std::endl;
		std::cout.flush();

		__timer_start();

		GUMIAndSongsDiv2 __object;
		int __actual = __object.maxSongs(duration, tone, T);
