...
vector <int>	keys;
		...
		__in >> __expected >> doorR >> doorG >> keys;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << doorR << ", " << doorG << ", " << keys << std::endl;
		std::cout.flush();

		__timer_start();

		KeyDungeonDiv2 __object;
		int __actual = __object.countDoors(doorR, doorG, keys);
