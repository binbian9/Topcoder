...
string	word;
		...
		__in >> __expected >> keyboard >> word;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << keyboard << ", " << word << std::endl;
		std::cout.flush();

		__timer_start();

		TypingDistance __object;
		int __actual = __object.minDistance(keyboard, word);
