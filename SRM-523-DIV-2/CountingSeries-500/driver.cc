...
long long	upperBound;
		...
		__in >> __expected >> a >> b >> c >> d >> upperBound;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << a << ", " << b << ", " << c << ", " << d << ", " << upperBound << std::endl;
		std::cout.flush();

		__timer_start();

		CountingSeries __object;
		long long __actual = __object.countThem(a, b, c, d, upperBound);
