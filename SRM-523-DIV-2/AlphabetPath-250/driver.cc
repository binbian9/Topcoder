...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> letterMaze;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << letterMaze << std::endl;
		std::cout.flush();

		__timer_start();

		AlphabetPath __object;
		string __actual = __object.doesItExist(letterMaze);
