...
int	b;
		...
		__in >> __expected >> n >> m >> j >> b;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << m << ", " << j << ", " << b << std::endl;
		std::cout.flush();

		__timer_start();

		TheBoredomDivTwo __object;
		int __actual = __object.find(n, m, j, b);
