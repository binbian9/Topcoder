...
#include "FoxAndFlowerShopDivTwo.cc"
...
int	c;
		...
		__in >> __expected >> flowers >> r >> c;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << flowers << ", " << r << ", " << c << std::endl;
		std::cout.flush();

		__timer_start();

		FoxAndFlowerShopDivTwo __object;
		int __actual = __object.theMaxFlowers(flowers, r, c);
